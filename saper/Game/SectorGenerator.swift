...
struct SectorGenerator {
    ...
    static func generate(at coord: SectorCoordinate, globalSeed: UInt64) -> Sector {
        let rng = SeededRandom.rng(globalSeed: globalSeed, sector: coord)
        let distance = coord.distanceFromOrigin
        let density = min(Constants.baseDensity + Double(distance) * Constants.densityMultiplier, Constants.maxDensity)
